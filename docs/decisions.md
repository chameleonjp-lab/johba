...
- 2026-06-12 Q3（R3 カメラ・演出／メインセッション実装）：視線を進行方向4m先へ・コーナーバンク（先6mとの向き差×1.4、上限±0.05rad×速度比、lerp追従）・updateCameraの毎フレームVector3生成を排除。ゴール前接戦スロモ（残り100m以内・最寄り未完走馬と1.5m未満で0.4倍速×実1.6s・1レース1回・完走後の早送りと相互排他・「接戦！」キュー表示）。タイトル画面を半透明化しアイドルカメラがコースを周回（racePhase==='idle'時）。ゴールフラッシュ（0.55白・0.18s）と1着時の紙吹雪（DOM30片・CSSアニメ・タイマー管理でリセット安全）
- 2026-06-12 Q4（R5+R6 体感・情報UI）：スタミナバー3色帯（帯変化時のみstyle更新・25%未満は点滅）、▲▼ムチの押下フィードバック（.pressedクラス）、バイブ（鞭15ms/掛かり発生40ms/ゴール30ms・navigator.vibrateガード付き）、初回操作ヒント（johba_hint_done・タップか5秒で消去）、予想人気（(4能力平均+相性×3)の順位。出走表・詳細モーダル・抽選カードの3箇所、pending.popularityで共有）、残距離マイルストーン800/600/200＋残り430mで「最後の直線！」（400は重複回避でスキップ）、着順表の着差（1着は絶対タイム・2着以下は+秒差）、起動スプラッシュ（module末尾で除去・12秒の非moduleフォールバック警告）、タイトルロゴのグラデーション文字化（text-shadow→drop-shadowへ変更）。HUD状態（帯・掛かりフラグ・マイルストーン集合）はopenGateScreen/resetAll両方でリセット
- 2026-06-12 Q5（R4 プロシージャルサウンド／メインセッション実装）：外部音源ファイルを一切使わず Web Audio のオシレータ／ノイズバッファで全効果音を合成。新設 `// ==== SECTION: AUDIO ====`（UI-SCREENS直前）に集約。遅延シングルトン（audioCtx/masterGain/crowdGain は1度だけ生成・gain値は全てCONFIG.SND_*）。初期化は `#btnStart` クリックと `handleGateTap` の最初のユーザー操作で `initAudio()`（自動再生制限／iOS対策に `audioCtx.resume()`）。効果音：①蹄音＝legPhaseを1ストライド4ビートに割りビート増分ごとにノイズ→ローパスを1打（最大~9打/秒・document.hidden/速度2未満/完走時スキップ）②ゲート開放＝矩形波220→90Hz＋高域ノイズバースト③鞭＝高域ノイズ鋭バースト＋900→500Hzピッチ落ち（タップ即発火・simが弾いても鳴る）④掛かり警告＝三角波880→988Hz2連ビープ⑤マイルストーンティック＝サイン1320Hz（最後の直線dur1000は1320→1760の2連）⑥歓声スウェル＝ループ中ノイズ→バンドパス（800Hz/Q0.6）専用crowdGainを残り400m未満で増大・running中は0.035・完走後は0.18から2秒でフェード（updateAudio内で毎フレーム×0.05平滑追従、ノード生成は蹄音のみ）⑦ゴール音＝着順別（1着＝523/659/784の上昇トライアド・2~3着＝2音・他＝392Hz単音）。全ワンショットは `audioCtx && soundOn` を確認しtry/catchで囲みゲーム本体を止めない（自動再生がブロックされ初期化が走らなくても無音で正常動作）。音ON/OFFトグルはタイトル `.sub-links` の `#btnSound` とレース中右上固定 `#btnSoundRace`（pointer-events:auto・z16・#hudと同じ4箇所＋resetAllで表示制御）。状態は localStorage `johba_sound`（既定on）・masterGainを0.1sでランプ・絵文字を避け「音ON／音OFF」表記。可視性変化で suspend/resume。SIM-COREは不変・sim-harness 3×1600=clean
